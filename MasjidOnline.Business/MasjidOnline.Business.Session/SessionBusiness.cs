...
public class SessionBusiness(IService _service, IIdGenerator _idGenerator) : ISessionBusiness
{
    ...
    public async Task ChangeAndSaveAsync(Interface.Session session, IData _data, int userId)
    {
    ...
    public async Task StartAsync(Interface.Session session, IData _data, string? idBase64, [CallerArgumentExpression(nameof(idBase64))] string? idBase64Expression = default)
    {
        if (idBase64 == default)
        {
