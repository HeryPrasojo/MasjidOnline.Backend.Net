...
public class GetTableBusiness(IService _service) : IGetTableBusiness
{
        ...
        TableRequest? getTableRequest)
    {
        ...
        var DonationStatuses = getTableRequest.Statuses?.Select(m => Mapper.Mapper.Donation.DonationStatus[m]);

        var take = 10;

        var getTableResult = await _data.Donation.Donation.GetTableAsync(
            paymentTypes: paymentTypes,
            paymentStatuses: DonationStatuses,
        ...
        return new()
        {
            ...
            Data = new()
            {
                ...
                Records = getTableResult.Records.Select(e => new TableResponseRecord
                {
                    ...
                    Status = _service.Localization[Mapper.Mapper.Donation.DonationStatus[e.Status], session.CultureInfo],
