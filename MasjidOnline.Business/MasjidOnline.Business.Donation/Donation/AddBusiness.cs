    ...
    IOptionsMonitor<BusinessOptions> _optionsMonitor) : IAddBusiness
{
    public async Task<Response> AddAsync(IData _data, Model.Session.Session session, AddRequest? addRequest)
    {
        ...
        var isCaptchaVerified = await _service.Captcha.VerifyDonationAsync(addRequest.CaptchaToken);

        if (!isCaptchaVerified) throw new InputMismatchException(nameof(addRequest.CaptchaToken));


        await _data.Transaction.BeginAsync(_data.Payment, _data.Donation);
        ...
        {
            Id = _data.IdGenerator.Donation.DonationId,
            ...
            Status = Entity.Donation.DonationStatus.New,
            UserId = session.UserId,
        };

        await _data.Donation.Donation.AddAsync(Donation);
        ...
        if (addRequest.PaymentType == PaymentType.ManualBankTransfer)
        {
            if (addRequest.Files != default)
            {
                foreach (var file in addRequest.Files)
                {
                    ...
                    var path = _optionsMonitor.CurrentValue.Directory.Donation + fileName;
                    var temporaryPath = _optionsMonitor.CurrentValue.Directory.Donation + '_' + fileName;
